...
def tasks_management():
    ...
    #nasłuch na metodzie POST (dodawanie notatki)
    if request.method == "POST":
        ...
        if len(note) <= 1:
            ...
            new_note = Note(data=note, title=title, user_id=current_user.id, deadline=deadline_object, status_id=status_object.id) #utworzenie nowej instancji klasy Note (notatki w RAM)
